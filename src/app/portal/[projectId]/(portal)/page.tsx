...
import { getProjectProgress, getUpcomingTasks, getProjectActivity } from '@/app/(app)/projects/progress-actions'
...
export default async function PortalOverview({
    ...
    const progress = await getProjectProgress(projectId, true) // Use admin client for portal access
    const upcomingTasks = await getUpcomingTasks(projectId, 5, true) // Use admin client for portal access
    const activities = await getProjectActivity(projectId, 10, true) // Use admin client for portal access
    ...
    const pendingQuestions = (progress?.totalQuestions || 0) - (progress?.answeredQuestions || 0)
    const pendingChecklists = (progress?.totalChecklists || 0) - (progress?.completedChecklists || 0)
    const pendingFiles = (progress?.totalFiles || 0) - (progress?.uploadedFiles || 0)
    const totalPending = pendingTasks + pendingQuestions + pendingChecklists + pendingFiles
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            {totalPending > 0 ? (
                <Card className="bg-white/95 backdrop-blur-sm overflow-hidden">
                    ...
                    {firstPage && (
                        <Link
                            href={`/portal/${projectId}/${firstPage.slug}`}
