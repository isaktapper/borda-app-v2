...
export async function getProjectProgress(projectId: string, useAdminClient: boolean = false): Promise<ProgressStats | null> {
...
export async function getProjectActivity(projectId: string, limit: number = 10, useAdminClient: boolean = false): Promise<ActivityItem[]> {
  const supabase = useAdminClient ? await createAdminClient() : await createClient()
...
export async function getUpcomingTasks(projectId: string, limit: number = 5, useAdminClient: boolean = false) {
  const supabase = useAdminClient ? await createAdminClient() : await createClient()
...
export async function getProjectStats(organizationId: string) {
  ...
  const lowEngagement = activeProjects.filter(
    p => p.engagement_level === 'low' || p.engagement_level === 'none'
  ).length

  // Calculate average engagement score
  const projectsWithScore = activeProjects.filter(p => p.engagement_score !== null)
  const avgEngagement = projectsWithScore.length > 0
    ? Math.round(
        projectsWithScore.reduce((sum, p) => sum + (p.engagement_score || 0), 0) / projectsWithScore.length
      )
    : 0

  // Count go-live this month
  const goLiveThisMonth = activeProjects.filter(p => {
...
export async function getDashboardStats(organizationId: string) {
  ...
  taskBlocks.forEach(block => {
    ...
    if (projectId) {
      blockTasks.forEach((task: any) => {
        ...
        if (status === 'pending' && task.dueDate && task.dueDate < today) {
          totalOverdueTasks++
          projectOverdueTasks.set(projectId, (projectOverdueTasks.get(projectId) || 0) + 1)
        }
      })
    }
  })
  ...
  projectOverdueTasks.forEach((overdueCount, projectId) => {
    const totalTasks = projectTaskCounts.get(projectId) || 0
...
export async function isFirstVisit(projectId: string, actorEmail: string): Promise<boolean> {
...
export async function logPortalVisitActivity(projectId: string, visitorEmail: string) {
  const firstVisit = await isFirstVisit(projectId, visitorEmail)
  const action = firstVisit ? 'portal.first_visit' : 'portal.visit'
  
  await logActivity(projectId, visitorEmail, action, 'portal', projectId, {
    firstVisit
  })
}
