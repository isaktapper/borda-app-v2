...
export function ProjectDetailClient({
    ...
    const handleRefreshEngagement = async () => {
        const updated = await refreshEngagementScore(projectId)
        if (updated) {
            ...
            <Tabs value={currentTab} onValueChange={handleTabChange} className="flex flex-col flex-1 overflow-hidden">
                <div className="border-b bg-background flex-none shrink-0">
                    ...
                    <div className="flex items-center justify-between py-4 border-b">
                        ...
                        <div className="flex items-center gap-2"><Button variant="outline" size="sm" asChild className="gap-2"><Link href={`/v2/projects/${projectId}`}><Sparkles className="size-4" />Try New Editor</Link></Button><PortalPreview projectId={projectId} projectName={project.client_name} /></div>
                    </div>
