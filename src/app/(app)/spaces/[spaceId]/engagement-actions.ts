...
export async function getProjectEngagement(spaceId: string): Promise<EngagementScoreResult | null> {
  ...
  try {
    const engagement = await calculateEngagementScore(spaceId)
...
export async function refreshEngagementScore(spaceId: string): Promise<EngagementScoreResult | null> {
  const supabase = await createClient()
