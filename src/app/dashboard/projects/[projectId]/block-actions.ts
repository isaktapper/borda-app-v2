...
export async function createTaskBlock(pageId: string, projectId: string, content: any, sortOrder: number) {
    const supabase = await createClient()
    ...
    const { error: taskError } = await supabase
        ...
        .insert({
            ...
            title: content.title || 'Namnlös uppgift',
...
export async function updateTaskBlock(blockId: string, projectId: string, content: any) {
    const supabase = await createClient()
...
export async function bulkUpdateBlocks(pageId: string, projectId: string, blocks: any[]) {
    const supabase = await createClient()
    ...
    for (const block of blocks) {
        ...
        if (isNew) {
            if (block.type === 'task') {
                await createTaskBlock(pageId, projectId, block.content, block.sort_order)
            } else {
            ...
            if (block.type === 'task') {
                await updateTaskBlock(block.id, projectId, block.content)
            } else {
