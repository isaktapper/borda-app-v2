...
export async function inviteToOrganization(
  ...
  await sendEmail({
    to: email,
    subject: `Inbjudan till ${organization?.name || 'en organisation'} på Impel`,
    html: orgInviteTemplate({
      organizationName: organization?.name || 'organisationen',
...
export async function resendInvitation(memberId: string) {
  ...
  await sendEmail({
    ...
    subject: `Påminnelse: Inbjudan till ${organization?.name || 'en organisation'} på Impel`,
    html: orgInviteTemplate({
      organizationName: organization?.name || 'organisationen',
      invitedByName: inviter?.full_name || inviter?.email || 'en kollega',
      inviteLink
